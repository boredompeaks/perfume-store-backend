...
| SPEC-6-03a | 6 | RBAC foundation I: six staff roles (support/catalogue/inventory/marketing/finance/admin) as Groups + capability→role mapping + idempotent seeding (makemigrations --check stays clean) + DRF capability permission classes in backend/common/permissions.py (extend the IsAdminUserOrReadOnly seam, no duplication) | 1425–2262 §6.12 lines 2099–2262 [6.12.2],[6.12.4],[6.12.7] | BUILDING (split from SPEC-6-03 after builder token exhaustion; task halved for manageable reasoning) | 1 |
| SPEC-6-03b | 6 | RBAC foundation II: apply capability permission classes to API views (no guard downgrades — everything staff-only stays at-least-as-restricted); role/permission changes restricted to admin role (privilege-escalation guard, spec line 2261) | 1425–2262 §6.12 lines 2099–2262 | PENDING (blocked by SPEC-6-03a) | 0 |
