...
| SPEC-6-04 | 6 | ModelAdmin least-privilege: has_view/change/delete/add overrides per role; bulk actions and exports role-gated; sensitive actions get confirmation | 1425–2262 [6.12.4],[6.12.5],[6.12.7] | IN-AUDIT (commit feb4a2e: common/admin.py RoleAwareModelAdmin + action-confirmation interstitial + all 6 ModelAdmins mapped; common/* folded into measured coverage (1182→1406 stmts), 1 exposed gap closed; 307 tests = 301 pass + 6 xfails, cov 100.00%; 3 ambiguous mappings flagged for auditor ruling) | 1 |
...
Current floor (AUDITOR-verified 2026-09-18, tree @ `099ad81`, SPEC-6-03a/b/c combined audit): **285 tests, OK (279 pass + 6 pre-existing expectedFailure), cov 100.00%** (1182 stmts, 0 miss). Floor only moves up: never fewer passing tests, never more expected failures, never lower coverage. NOTE (auditor follow-up, fold into SPEC-6-04): pre-existing `.coveragerc` omits `common/*` from the measured set — fold it in during SPEC-6-04 and close any exposed gaps in the same commit (measured floor must stay >= 100.00% of the larger set or be honestly re-baselined with evidence).
