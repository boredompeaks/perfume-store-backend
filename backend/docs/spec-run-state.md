...
| SPEC-2-01 | 2 | DATABASES hardcoded sqlite3 (settings.py:92-97) — make env-driven (e.g. dj-database-url) with sqlite dev fallback; Postgres provisioning itself stays deferred — Owner: build now; deployment/FTS deferred to S22/S9 | 152–278 [2.4] | IN-AUDIT (gate re-verification cycle-2 pending: builder fix 0e0ed17 replaces password-shaped literal with `unquote('p%40ss')` derived oracle, semantics preserved; floor held 261/255 pass/6expF/100.00% run twice; changelog row appended; awaiting RE re-promote) | 2 |
