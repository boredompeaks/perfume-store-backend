...
|---|---|---|---|---|---|
| SPEC-19-1 | 19 | P2 — DEVIATES [R-19.0]: notifications are scattered manual send_mail calls inside controllers (accounts/views.py:75-76 et al; zero order emails) — build an in-process notification/email service + AuditEvent-subscribing dispatcher (single send path, no queue — explicitly NOT SPEC-2-03's async work), prerequisite substrate for SPEC-1-12/SPEC-3-14 email content | 4603–4777 [R-19.0] + S1[1.12] | PENDING (P2 — PULL-FORWARD candidate: unblocks S10/S19 order emails) | 0 |
