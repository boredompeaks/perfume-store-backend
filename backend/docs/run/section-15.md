...
|---|---|---|---|---|---|
| SPEC-15-1 | 15 | P2 — WCAG AA failures [R-15.4.1/R-15.4.7]: (a) placeholder text ≈3.6:1 < 4.5:1 (placeholder:text-ink-muted/70 — FilterBar.tsx:19-20+83,97,109, CouponForm.tsx:10-11+140); (b) input borders border-line #e6e1d7 ≈1.2-1.3:1 < 3:1 WCAG 1.4.11 (every text input via shared inputClass). Fix: drop the /70 modifier, darken border-line or add a field-border token, ideally centralizing the duplicated inputClass into one shared field class | 3855–4088 [R-15.4.1],[R-15.4.7] | PENDING (P2 build-now — QUEUED after the S7 queue drains, before the S8 queue; first frontend build task of the run) | 0 |
